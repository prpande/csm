...
import { isUnderAnyRoot, filterOutTemp } from "../../src/sessionFilter";
...
describe("isUnderAnyRoot", () => {
  test("matches a cwd under a Windows temp root, case-insensitively", () => {
    expect(
      isUnderAnyRoot("C:\\Users\\p\\AppData\\Local\\Temp\\abc", [WIN_TEMP]),
    ).toBe(true);
    // Case folds on Windows.
    expect(
      isUnderAnyRoot("c:\\users\\P\\appdata\\local\\temp\\abc", [WIN_TEMP]),
    ).toBe(true);
  });
  ...
  test("matches POSIX roots exactly at the boundary", () => {
    expect(isUnderAnyRoot("/var/folders/xy/session", ["/var/folders"])).toBe(
      true,
    );
    expect(isUnderAnyRoot("/home/me/proj", ["/tmp", "/var/folders"])).toBe(
      false,
    );
  });
